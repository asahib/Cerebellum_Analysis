...
groupzstatdir="${maindir}/PALM3rdLevel/FM/paper2/KTP2_KTP3"
...
#Cortical ROIs:
ROIlist="R_posteriorcingulate R_caudalanteriorcingulate R_lateraloccipital R_precuneus R_fusiform R_superiortemporal R_insula R_rostralmiddlefrontal R_caudalmiddlefrontal R_postcentral"
...
wb_command -cifti-math 'x > 1.3' ${FuncMasksdir}/${FuncMaskname}.dscalar.nii -var 'x' ${groupzstatdir}/${CopeMask}/results_dense_tfce_ztstat_${corrp}_${ConMask}.dscalar.nii
wb_command -cifti-create-dense-from-template ${hcpdir}/k000901/MNINonLinear/Results/task-facematching_acq-AP_run-01/task-facematching_acq-AP_run-01_Atlas_mean.dscalar.nii ${FuncMasksdir}/${FuncMaskname}.dscalar.nii -cifti ${FuncMasksdir}/${FuncMaskname}.dscalar.nii
fi
...
then 
	...
	wb_command -cifti-create-label ${templatesdir}/Atlas_ROIs_template.2.dlabel.nii -volume ${templatesdir}/Atlas_ROIs.2.nii ${templatesdir}/Atlas_ROIs.2.nii -left-label ${hcpdir}/k000801/MNINonLinear/fsaverage_LR32k/sub-k000801.L.aparc.32k_fs_LR.label.gii -right-label ${hcpdir}/k000801/MNINonLinear/fsaverage_LR32k/sub-k000801.R.aparc.32k_fs_LR.label.gii
	
	wb_command -cifti-create-dense-from-template ${hcpdir}/k000901/MNINonLinear/Results/task-facematching_acq-AP_run-01/task-facematching_acq-AP_run-01_Atlas_mean.dscalar.nii ${templatesdir}/Atlas_ROIs_template.2.dlabel.nii -cifti ${templatesdir}/Atlas_ROIs_template.2.dlabel.nii
...
do
	...
	then
		...
		if [ ${MergeROIs} = "YES" ]
		then
			wb_command -cifti-stats ${betaimgsdir}/${sub}_task-facematching_JL_20cons_clean_level2_percChange_${cope}_hp200_s5.dscalar.nii -reduce MEAN -roi ${CombinedMasksdir}/MERGEDROIs_${FuncMaskname}.dscalar.nii >> ${logdir}/${cope}/MERGEDROIs_${cope}_${FuncMaskname}_v2ALLROIs.txt

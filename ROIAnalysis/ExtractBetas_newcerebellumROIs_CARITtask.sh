...
do
	...
	logfile="${logdir}/MERGEDROIs_Cortex_AnticevicNetworks_carit${cope}_${PPIROI}.txt"
	if [ -f ${logfile} ]; then
		rm ${logfile}
	fi
	...
	do
		...
		wb_command -cifti-stats ${betaimgsdir}/${cope}.dtseries.nii -reduce MEAN -roi ${ROIMasksdir}/MERGEDROIs_AnticevicNetworks.dscalar.nii >> ${logfile}

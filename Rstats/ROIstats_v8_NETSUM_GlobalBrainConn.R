...
for (roi in 1:length(PPIROI)) {
  ...
  for (h in 1:length(Hemi)) {
    ...
    Betas <- read.delim(paste("Rtest/Betas/SUM_ALLNetsin1_AvNoGoGo_",PPIROI[roi],"_",Hemi[h],".txt",sep=""), header=FALSE)
    ...
    Betas_4Analysis_abs<-abs(Betas_4Analysis %>% select(2))
    ClinicalData_4Analysis_wBetas<-cbind2(ClinicalData_4Analysis_matchBetaSubs,Betas_4Analysis_abs)
    ...
    for (i in 1:(dim(Betas)[2]-1)) {
      ...
      tmp<-cbind(tmp1, SeedROI1,SeedROIcode1,Hemcol1,Netcol3,Netcode3,Table1_ClinicalData_4Analysis_wBetas_tmp[32+i])
      colnames(tmp)<-c(colnames(Table1_ClinicalData_4Analysis_wBetas_tmp %>% select(1:29)),c("SeedROI","SeedROIcode","Hemi","Network","Networkcode","Betas"))
...
write.csv(Table1_ClinicalData_4Analysis_wBetas,paste("Rtest/stats/Table1_GlobalBrainConn_Betas_wide.csv",sep=""))
write.csv(Table2_ClinicalData_4Analysis_wBetaschange,paste("Rtest/stats/Table2_GlobalBrainConn_BetasChange_wide.csv",sep=""))
write.csv(Table3_Long,paste("Rtest/stats/Table3_GlobalBrainConn_Betas_Long.csv",sep=""))
